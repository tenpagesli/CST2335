package com.cst2335;

public class MyUtil {
    /** common fields */
    public static final String versionNumber = "1.0";

    /** help dialog contents for the app*/
    public static final String appAuther = "Dictionary: Zhe (Ryan) Li<br/>" +
                                            "Flight Status Tracker: Yan (Queeny) Qu<br/>" +
                                            "News Feed: Hung Doan<br/>" +
                                            "New York Times Article: Kevin Nghiem<br/><br/>";
    public static final String appVersion = "Version 1.0<br/>";

    /** help dialog contents for Ryan*/
    public static final String dictionaryAuther = "Zhe (Ryan) Li";
    public static final String dictionaryInstruction = "Here is a long instruction";

    /** api key for Queeny */
    public static final String flightApiKey = "8639b0-1eaa4a";
    /** help dialog contents for Queeny*/
    public static final String flightAuther = "Yan (Queeny) Qu";
    public static final String flightInstruction = "Queeny's meodule instruction";
}
